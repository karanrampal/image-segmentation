...
class Params():
    """Class to load hyperparameters from a json file or a dictionary.
    """
    def __init__(self, inp):
        if isinstance(inp, dict):
            self.update_dict(inp)
        elif isinstance(inp, str):
            self.update_file(inp)
        else:
            raise RuntimeError(
            f"Invalid input, Input should either be a dictionary or a string path to a config file!"
        )
    ...
    def update_file(self, json_path):
        """Loads parameters from json file at json_path
        """
